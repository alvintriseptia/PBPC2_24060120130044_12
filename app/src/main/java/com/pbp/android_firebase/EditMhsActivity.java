...
public class EditMhsActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        databaseReference = FirebaseDatabase.getInstance("https://praktikum-firebase-a0e14-default-rtdb.asia-southeast1.firebasedatabase.app").getReference("mhs").child(key);
        databaseReference.addValueEventListener(new ValueEventListener() {
            @Override
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                Log.i("EditData", "onDataChange: " + dataSnapshot.toString());
                temp_nim = dataSnapshot.child("nim").getValue() + "";
                temp_nama = dataSnapshot.child("nama").getValue() + "";
                temp_jurusan = dataSnapshot.child("jurusan").getValue() + "";
                temp_angkatan = dataSnapshot.child("angkatan").getValue() + "";
